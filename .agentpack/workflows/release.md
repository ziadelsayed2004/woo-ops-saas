...
- Woo fixture certification, tenant isolation, security and critical E2E pass.
...
3. Deploy API.
4. Deploy workers with reduced initial concurrency.
5. Deploy web.
6. Run auth, test-store health, webhook, query, manual order, export and PDF smoke tests.
7. Observe lag/errors/latency/resources, then raise worker concurrency.
...
Monitor per-connection lag, webhook rejects, queue age, duplicate/idempotency rate, Mongo query
latency, storage/PDF failures, authentication anomalies, cross-tenant security alarms, and support
