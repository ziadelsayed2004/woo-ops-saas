...
export function validatePack({ includeState = true } = {}) {
  ...
  assert(manifest.pack?.id === registry.project, "Manifest pack ID must match task registry project", errors);
  assert(manifest.product?.connectorDirection === "read-only", "Connector direction must remain read-only", errors);
  assert(manifest.product?.manualOrders === "local-only", "Manual orders must remain local-only", errors);
  assert(manifest.product?.database === "mongodb-atlas", "Production database decision must remain mongodb-atlas", errors);
  assert(manifest.sourceControl?.oneTaskPerWorktree === true, "oneTaskPerWorktree must be true", errors);
  assert(Array.isArray(manifest.qualityGates) && manifest.qualityGates.length > 0, "Quality gates are required", errors);

  assert(registry.schemaVersion === 1, "registry.schemaVersion must be 1", errors);
  assert(Array.isArray(registry.tasks) && registry.tasks.length > 0, "Task registry must contain tasks", errors);
  ...
  for (const task of registry.tasks) {
    ...
    assert(typeof task.title === "string" && task.title.length >= 5, `Task ${task.id} needs a title`, errors);
    ...
    assert(typeof task.parallelGroup === "string" && task.parallelGroup.length > 0, `Missing parallelGroup for ${task.id}`, errors);
    ...
    for (const reference of task.references ?? []) {
      ...
      assert(absolute.startsWith(repositoryRoot + path.sep), `Task ${task.id} has unsafe reference: ${reference}`, errors);
      assert(fs.existsSync(absolute), `Task ${task.id} references missing file: ${reference}`, errors);
    }
  }
  ...
  for (const task of registry.tasks) {
    for (const dependency of task.dependencies ?? []) {
      ...
      if (dependencyTask && dependencyTask.phase > task.phase) {
        warnings.push(`Task ${task.id} phase ${task.phase} depends on later phase ${dependencyTask.phase} task ${dependency}`);
      }
    }
  }
  ...
  if (includeState) {
    ...
    for (const [taskId, taskState] of Object.entries(state.tasks)) {
      ...
      assert(registry.statuses.includes(taskState.status), `Runtime task ${taskId} has invalid status ${taskState.status}`, errors);
    }
  }

  const ready = state
    ? registry.tasks.filter((task) => displayTaskStatus(task, state) === "ready").map((task) => task.id)
...
export function printValidation(result) {
  ...
  if (result.ok) {
    console.log(`Agent pack valid: ${result.summary.tasks} tasks, ${result.summary.filesChecked} required files.`);
